...
(define value-of-ds
  (λ (e env)
    (match e
      ...
      [`,y
       #:when (symbol? y)
       (unbox (apply-env-ds env y))]
      [`(lambda (,x) ,body)
       #:when (symbol? x)
       (λ (a) (value-of-ds body (extend-env-ds x a env)))]
      [`(let ((,x ,e1)) ,body)
       #:when (symbol? x)
       (value-of-ds body (extend-env-ds x (box (value-of-ds e1 env)) env))]
      [`(+ ,e1 ,e2)
      ...
      [`(set! ,x ,exp)
       ...
       (let ((tmp (value-of-ds exp env)))
         (set-box! (apply-env-ds env x) tmp))]
      [`(,rator ,x)
       #:when (symbol? x)
       ((value-of-ds rator env) (box (unbox (apply-env-ds env x))))]
      [`(,rator ,rand)
       ((value-of-ds rator env) (box (value-of-ds rand env)))])))
...
(define value-of-fn
  (λ (e env)
    (match e
      ...
      [`,y
       #:when (symbol? y)
       (unbox (apply-env-fn env y))]
      [`(lambda (,x) ,body)
       #:when (symbol? x)
       (λ (a) (value-of-fn body (extend-env-fn x a env)))]
      [`(let ((,x ,e1)) ,body)
       #:when (symbol? x)
       (value-of-fn body (extend-env-fn x (box (value-of-fn e1 env)) env))]
      [`(+ ,e1 ,e2)
      ...
      [`(begin2 ,e1 ,e2) (begin (value-of-fn e1 env) (value-of-fn e2 env))]
      [`(set! ,x ,exp)
       #:when (symbol? x)
       (let ((tmp (value-of-fn exp env)))
         (set-box! (apply-env-fn env x) tmp))]
      [`(,rator ,x)
       #:when (symbol? x)
       ((value-of-fn rator env) (box (unbox (apply-env-fn env x))))]
      [`(,rator ,rand)
       ((value-of-fn rator env) (box (value-of-fn rand env)))])))
...
;brainteasers
;5: modify code in value-of-ds and value-of-fn: basic idea: use box to make set! possible
