  ...
  (lambda (x2 env-cps k)
    (lambda (v1)
      (value-of-cps x2 env-cps
                    (make-mult-x2-continuation v1 k)))))
...
(define make-throw-k-continuation
  (lambda (v-exp env-cps)
    (lambda (k-rator)
      (value-of-cps v-exp env-cps (make-throw-v-continuation k-rator)))))

(define make-let-continuation
  (lambda (body env-cps k)
    (lambda (e-val)
      (value-of-cps body (extend-env e-val env-cps) k))))
...
(define make-rator-continuation
  (lambda (rand env-cps k)
    (lambda (clos)
      (value-of-cps rand env-cps (make-rand-continuation clos k)))))
